...
XHRConnection.prototype.flush = function() {
  ...
  if(this.timeout)xreq.timeout = this.timeout;
  if(this.timeoutHandler)xreq.ontimeout = (this.timeoutHandler)();
  ...
  Object.keys(this.headers).forEach(function(headerKey) {
    ...
    this.send_buf = new Buffer(JSON.parse(this.send_buf.toString().replace(/\\/g,""), 'binary'));
	 console.log("XHRConnection.prototype.flush",this.send_buf.toString());
